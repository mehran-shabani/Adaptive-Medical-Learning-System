...
from app.mastery.schemas import (
    MasteryScore, UserMasteryDashboard, MasteryUpdate, TopicMasteryDetail
)
...
class MasteryService:
    ...
    @staticmethod
    def get_user_mastery_dashboard(
        user_id: int,
        db: Session
    ) -> UserMasteryDashboard:
        """
        ...
        topic_map = {
            ...
            db.query(Topic).filter(Topic.id.in_([m.topic_id for m in masteries])).all()
        }
        ...
        for m in masteries:
            ...
            if topic:
                mastery_scores.append(MasteryScore(
                    topic_id=m.topic_id,
        ...
        recent_activity = sorted(
            [m for m in mastery_scores if m.last_reviewed_at],
            key=lambda x: x.last_reviewed_at,
            reverse=True
        )[:10]
    ...
    def get_topic_mastery_detail(
        ...
        quiz_answers = db.query(QuizAnswer).join(QuizQuestion).filter(
        ...
        elif mastery.last_reviewed_at and days_since(mastery.last_reviewed_at) > settings.SPACED_REPETITION_THRESHOLD_DAYS:
            needs_review = True
    ...
    @staticmethod
    def get_weak_topics_for_review(
        user_id: int,
        limit: int,
        db: Session
    ) -> List[Mastery]:
